...
Looking at the build logs and build timeline available in Xcode 14, I see that from the 10 minutes it takes to compile my project, around half of the time is spent building external dependencies. Taking in account that I don't change the versions of those dependencies that frequently, wouldn't it be great not to recompile them? Especially annoying here for me personally is that Firebase is always rebuilding the protobuf library every time. Wait, I am sure I've seen this before. Was it in the Simpsons?

## Reinventing carthage

No, it was not in the Simpsons, but I can clearly remember it. [Carthage](https://github.com/Carthage/Carthage) allows this. Carthage is another package management tool that was created before SwiftPM. Carthage is building external dependencies and packaging them as binary XCFrameworks. Why not use it?

## High-level overview

We'll move from using external dependencies compiled by SPM on the build time to using pre-compiled versions of the same dependencies:
...
Carthage is going to put the frameworks to `Carthage/Build`. This is where we can take them and archive them:
...
We need to think of other engineers who would be interacting with the repository, so it's a good idea to create a setup script that would make sure Git LFS extension is installed and would pull the dependencies file:

```bash
echo "Installing binary third-party libraries"
brew install git-lfs
git lfs install
git lfs pull
...
QQ: Should one consider switching to Carthage alltogether?

In my opinion no, because Swift PM allows for modularization.

QQ: Why use XCFramework format, specifically?

This format, in comparison to a regular `framework`, allows the packaging of several binaries of the same architecture for different SDKs. In practise, this means Apple Silicon simulator support.
...
- Dynamic frameworks negatively affect your app's startup time. iOS is taking some time to load dynamic libraries on the app starup.

<!-- Why run tests for the code you haven't changed?  -->